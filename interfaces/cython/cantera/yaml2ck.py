...
def reorder_reaction_equation(solution, reaction):
    ...
    reaction_txt = []
    reaction_split = {"reactants": reaction.reactants, "products": reaction.products}
...
def build_arrhenius(rate, reaction_order, reaction_type):
    ...
    arrhenius = [
        f"{eformat(pre_exponential_factor)}",
...
def thermo_notes(solution_species):
    comment = {}
    note = {}
    for species in solution_species:
        if "note" in species.input_data["thermo"]:
            note_raw = species.input_data["thermo"]["note"]
        else:
            note_raw = ""
...
def build_nasa7(solution_species):
    ...
    for species in solution_species:
        ...
        species_string = (
            ...
            + "\n"
            + f"{name_and_note:<24}"
            + f"{composition_string:<20}"  # name and date/note field
            + "G"
            + f"{species.thermo.min_temp:10.3f}"  # only supports gas phase
        ...
        species_string += (
            "".join([f"{c:15.8e}" for c in species.thermo.coeffs[11:15]]) + f"{4:>20}\n"
        )
...
def build_nasa9(solution_species):
    # Ex:
    #   therm NASA9
    ...
    # NASA9 not supported by Chemkin currently though? Comes from NASA CEA mostly
    raise NotImplementedError(f"Unsupported thermo model: NASA9")
...
def species_data_text(solution_species):
    ...
    if any(note.values()):  # check that any notes exist
        ...
        species_txt = []
        for species in solution_species:
            text = f"{species.name:<{max_species_len}}   ! {note[species.name]}\n"
...
def thermo_data_text(solution_species, input_type="included"):
    ...
    species_thermo_model = {}
    for species in solution_species:
        species_thermo_model[species.name] = species.input_data["thermo"]["model"]

    # Write thermo
    if all([model == "NASA7" for model in species_thermo_model.values()]):
        if input_type == "included":
            thermo_text = ["THERMO ALL\n" + "   300.000  1000.000  6000.000\n"]
        else:
            thermo_text = ["THERMO\n" + "   300.000  1000.000  6000.000\n"]

        thermo_text.extend(build_nasa7(solution_species))
        ...
        thermo_text.extend(build_nasa9(solution_species))
...
def write_transport_data(
    species_list, filename="generated_transport.dat"
):  # need to add notes
    ...
    with open(filename, "w") as trans_file:
        ...
        for species in species_list:
            ...
            species_string = (
                ...
                + f"{(species.transport.dipole / DEBYE_CONVERSION):>10.3f}"
...
def write(
    ...
    separate_thermo_file : bool, optional
    ...
    with open(output_path, "w") as mech_file:
        ...
        if "description" in solution.input_header:
            note = solution.input_header["description"]
            mech_file.write(f"! {note}\n!\n")
        else:
            note = ""

        if "! Chemkin file converted from Cantera solution object" not in note:
            mech_file.write(
        ...
        for n, reaction in enumerate(solution.reactions()):
            ...
            if "note" in reaction.input_data:
                rxn_note = [
                    f"! {note.strip()}"
                    for note in reaction.input_data["note"].rsplit("\n")
                ]
                if len(rxn_note) > 1:
                    reaction_string += "\n".join(rxn_note[:-1]) + "\n"
            ...
            if type(reaction) in [ct.ElementaryReaction, ct.ThreeBodyReaction]:
                arrhenius = build_arrhenius(
                    reaction.rate, sum(reaction.reactants.values()), type(reaction)
                )
                ...
                raise NotImplementedError(
                    f"Unsupported reaction type: {type(reaction)}"
                )
            ...
            if type(reaction) == ct.FalloffReaction:
                ...
                if reaction.falloff.parameters.size > 0:
                    falloff_str = build_falloff(
                        reaction.falloff.parameters, reaction.falloff.type
                    )
                    width = (
                        max_rxn_width - 10 - 15 * (reaction.falloff.parameters.size - 3)
                    )
                ...
                for rate in reaction.rates:
                    ...
                    arrhenius = build_arrhenius(
                        rate[1], sum(reaction.reactants.values()), ct.PlogReaction
                    )
                    reaction_string += (
                        f'{"PLOG / ".rjust(max_rxn_width-18)}'
    ...
    # write thermo data
    if not skip_thermo and separate_thermo_file:
...
def main(argv):
    ...
    try:
        ...
        if args:
            raise getopt.GetoptError(
                "Unexpected command line option: " + repr(" ".join(args))
            )
    ...
    separate_thermo_file = "--separate_thermo_file" in options
