...
def build_nasa7(solution_species):
    ...
    note, comment = thermo_notes(solution_species) # get notes and comments for all species

    thermo_text = []
    # write data for each species in the Solution object
    for species in solution_species:
        composition_string = ''.join([f'{s:2}{int(v):>3}' for s, v in species.composition.items()])
        
        name_and_note = f'{species.name} {note[species.name]}'
        species_string = (comment[species.name] + '\n' +
...
def thermo_data_text(solution_species, input_type='included'):
    ...
    if all([model == 'NASA7' for model in species_thermo_model.values()]):
        ...
        if input_type == 'included':
            thermo_text = ['THERMO ALL NASA9\n' +  
                        '   200.000  1000.000  6000.000  20000.000\n']
        else:
            thermo_text = ['THERMO NASA9\n' +  
                        '   200.000  1000.000  6000.000  20000.000\n']
...
def write(solution, output_path='', reorder_reactions=False,
          skip_thermo=False, separate_thermo_file=False, 
    ...
    with open(output_path, 'w') as mech_file:
        ...
        for n, reaction in enumerate(solution.reactions()):
            ...
            if 'note' in reaction.input_data:
                rxn_note = [f'! {note.strip()}' for note in reaction.input_data['note'].rsplit('\n')]
                if len(rxn_note) > 1:
                    reaction_string += '\n'.join(rxn_note[:-1]) + '\n'
...
def convert_mech(input, output_path='', reorder_reactions=True,
    ...
    return write(solution, output_path, reorder_reactions,
                 skip_thermo, separate_thermo_file, skip_transport)


def main(argv):

    longOptions = ['input=', 'output_path=', 'reorder_reactions=', 'skip_thermo', 
                   'separate_thermo_file', 'skip_transport', 'help', 'debug',
                   'no-validate']
